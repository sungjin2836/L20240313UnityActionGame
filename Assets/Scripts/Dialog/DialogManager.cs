...
{
    List<DialogData> _dialogQueue; //리스트에서 들어온 순서대로 유저에게 보여주기 위함
...
//    Ranking
//}
////상속 못받게 제한 거는 키워드 sealed - 싱글톤이라 상속 못받게 함
////런 타임 시 특정 함수 호출할 경우 부모, 자식 클래스를 전체 조사해
////최종적으로 사용할 함수를 찾게 됨
...
//        get
//        {
//            return instance; //자기자신을 돌려쓰기 위해 사용
//        }
//    }


//    //생성 시 다이얼로그 큐와 다이얼로그 맵을 초기화함
//    private DialogManager()
//    {
//        _dialogQuere = new List<DialogData>();
//        _dialogMap = new Dictionary<DialogType, DialogController>();
//    }
//    #endregion
...
//    {
//        //딕셔너리명[키] = 값;
//        //해당 키를 가진 값이 만들아짐(키 : 값)
//        _dialogMap[type] = controller;
//    }

//    //데이터를 최상단에 저장하는 행위
//    //다이얼로그 리스트를 저장하는 다이엃로그 큐에 새로운 다이얼로그 데이터를 추가하는 행위
//    public void Push(DialogData data)
//    {
//        _dialogQuere.Add(data);
...
//                //익명 delegate
//                //delegate(매개변수 목록){실행하고자 하는 코드};
//                //함수이름 없이 기능만 델리게이트에 할당하기 위한 수단
//                _currentDialog.Close(
//                    delegate
//                    {
//                        _currentDialog = null;
//                        if(_dialogQuere.Count > 0 )
//                        {
//                            ShowNext();
//                        }
//                    }
...
//    {
//        //다이얼로그를 리스트에서 첫번째 값을 가져오겠습니다.
//        var next = _dialogQuere[0];

//        //가져온 값의 형태를 확인해 어떤 컨트롤러인지를 확인
//        var controller = _dialogMap[next.Type].GetComponent<DialogController>();

//        //조회한 다이얼로그 컨트롤러의 현재의 다이얼로그 컨트롤러로 지정함
//        _currentDialog = controller;
//        //현재의 다이얼로그를 빌드하겠습니다.
//        _currentDialog.Build(next);
//        //다이얼로그를 화면에 보여주는 작업
//        _currentDialog.Show(delegate { });
//        //다이얼로그 리스트에서 꺼내온 데이터를 제거함
//        _dialogQuere.RemoveAt(0);
//        //_dialogQuere.Remove(next);
//    }

//    //현재 팝업 창이 표시되고 있는지를 확인하는 변수
//    //_current가 비어있는 경우는 없다고 판단함
//    public bool IsShowing() => _currentDialog != null;
//}
